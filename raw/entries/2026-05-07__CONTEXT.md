...
DB run state is primary. `runWorkflow` inserts `_smithers_runs` and input rows (`engine.js` lines 4938-5050); `loadOutputs` reads per-schema tables by `runId` (`snapshot.js` lines 84-127). NDJSON logs are observability, not identity.
...
- Do not try to make `runs/` canonical. If the current browser needs a transitional `RenderGraph` JSON response, serve it from an API response, not as public `runs/<runId>` contract.
