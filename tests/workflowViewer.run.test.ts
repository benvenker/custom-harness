...
function expectWorkflowRunCommand(
  command: SmithersCliCommand,
  projectRoot: string,
  input: Record<string, unknown>
) {
...
describe("Smithers CLI command resolution for project runs", () => {
  it("prefers the project-root local smithers bin and runs from projectRoot", async () => {
    ...
    const command = buildSmithersWorkflowRunCommand({
      projectRoot,
      workflowId: "foo",
      input,
    });
...
describe("project workflow run API", () => {
  it("launches the selected workflow through the runner seam and returns Smithers run status plus inspection URL", async () => {
    ...
    const response = await handler(
      new Request("http://localhost/api/workflows/foo/run", {
        method: "POST",
  ...
  it("rejects project-mode prompt overrides before invoking the project runner", async () => {
