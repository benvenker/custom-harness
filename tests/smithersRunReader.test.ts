...
const runMainFrameXmlJson = JSON.stringify(runMainFrameXml);
const runMainTaskIndex = {
  draft: { nodeId: "draft", label: "Draft answer" },
  review: { nodeId: "review", label: "Review answer" },
};
const runMainTaskIndexJson = JSON.stringify(runMainTaskIndex);
const deltaFrameXml = {
  ...runMainFrameXml,
  children: [
    {
...
const deltaFrameXmlJson = JSON.stringify(deltaFrameXml);
const reviewFrameDeltaJson = JSON.stringify({
  version: 1,
  ops: [
    {
      op: "set",
      path: ["children", 0, "props", "id"],
      value: "review",
      nodeId: "review",
    },
  ],
});
const reviewFrameTaskIndexJson = JSON.stringify([
  { nodeId: "review", ordinal: 0, iteration: 0 },
]);
...
function createSmithersMappingFixtureDb(projectRoot: string) {
  ...
  try {
    ...
    insertRun.run(
      ...
      null,
      "delta-frame",
      join(projectRoot, ".smithers/workflows/delta-frame.tsx"),
      "hash-delta",
      "finished",
      600,
      610,
    ...
    insertEvent.run(
      ...
      2210,
      "attempt_finished",
      '{"nodeId":"review","iteration":1,"attempt":2,"ok":false}'
    );
    ...
    insertFrame.run(
      ...
      2,
      640,
      reviewFrameDeltaJson,
      "xml-hash-delta-2",
      "delta",
      '["review"]',
      reviewFrameTaskIndexJson,
      "delta update render"
    );

    db.query(
      "INSERT INTO result_output (run_id, node_id, iteration, markdown, score) VALUES (?, ?, ?, ?, ?)"
...
function createSmithersRunsFixtureDb(
  projectRoot: string,
  runs: SmithersRunFixture[]
) {
  const dbPath = smithersDbPath(projectRoot);
  const db = new Database(dbPath);
  try {
    ...
    for (const run of runs) {
      insertRun.run(
        ...
        run.workflowName,
        run.workflowPath ??
          join(projectRoot, `.smithers/workflows/${run.workflowName}.tsx`),
        `hash-${run.runId}`,
        run.status ?? "finished",
        run.createdAtMs,
        run.status === "running" || run.status === "continued"
          ? run.createdAtMs + 1
          : null,
...
describe("Smithers read-only SQLite opener", () => {
  ...
  it("fails with SMITHERS_DB_NOT_FOUND from a nested search start without creating any smithers.db files", async () => {
    const projectRoot = tempProject(
      "custom-harness-smithers-nested-missing-db-"
    );
    ...
    for (const dbPath of possibleCreatedDbPaths) {
      expect(
        existsSync(dbPath),
        `${dbPath} should not exist before opening`
      ).toBe(false);
    }

    const { openSmithersDbReadOnly } = await loadReadOnlyOpener();
    const error = await captureError(() =>
      (openSmithersDbReadOnly as any)({
    ...
    for (const dbPath of possibleCreatedDbPaths) {
      expect(
        existsSync(dbPath),
        `${dbPath} should not be created by read-only discovery`
      ).toBe(false);
    }
  });
  ...
  it("resolves the nearest parent smithers.db from a nested workflow-like dbSearchStart and remains read-only", async () => {
    ...
    try {
      ...
      await expect(
        maybeAsync(handle.queryAll("SELECT value FROM probe ORDER BY id"))
      ).resolves.toEqual([{ value: "parent-db" }]);
      ...
      for (const sql of [
        ...
        expect(error).toBeInstanceOf(Error);
        expect(String((error as Error).message)).toMatch(
          /readonly|read-only|query_only|not authorized|write/i
        );
      }
    } finally {
      handle.close();
  ...
  it("uses an explicit dbPath before projectRoot discovery when both databases exist", async () => {
    ...
    const explicitDbPath = createWritableFixtureDb(
      explicitDbRoot,
      "explicit-db"
    );
    ...
    try {
      ...
      await expect(
        maybeAsync(handle.queryAll("SELECT value FROM probe ORDER BY id"))
      ).resolves.toEqual([{ value: "explicit-db" }]);
    } finally {
      handle.close();
    }
  });
  ...
  it("close() releases the read-only connection and prevents further probe use", async () => {
    ...
    await expect(
      maybeAsync(handle.queryAll("SELECT value FROM probe"))
    ).resolves.toHaveLength(1);
    handle.close();

    const error = await captureError(() =>
      handle.queryAll("SELECT value FROM probe")
    );
    expect(error).toBeInstanceOf(Error);
  });
...
describe("SmithersRunReader contract", () => {
  ...
  it("does not manually write to Smithers internal tables from project-mode code", () => {
    ...
    const forbiddenSmithersWrite =
      /\b(?:INSERT(?:\s+OR\s+\w+)?\s+INTO|UPDATE(?:\s+OR\s+\w+)?\s+|DELETE\s+FROM|CREATE(?:\s+TEMP(?:ORARY)?)?\s+(?:TABLE|INDEX|TRIGGER|VIEW)|DROP\s+(?:TABLE|INDEX|TRIGGER|VIEW)|ALTER\s+TABLE)\s+[`'\"]?_smithers_/i;
...
describe("SmithersRunReader data mapping", () => {
  it("listRuns({ workflowId, limit }) applies the workflow filter before limiting rows", async () => {
    const projectRoot = tempProject(
      "custom-harness-smithers-filter-before-limit-"
    );
    ...
    try {
      ...
      expect(fooRuns.map((run) => run.runId)).toEqual([
        "foo-newest-match",
        "foo-second-newest-match",
      ]);
    } finally {
      reader.close();
    }
  ...
  it("listRuns({ workflowId, status: running }) includes continued runs for the matching workflow", async () => {
    const projectRoot = tempProject(
      "custom-harness-smithers-running-continued-"
    );
    createSmithersRunsFixtureDb(projectRoot, [
      {
        runId: "foo-running",
    ...
    const reader = await maybeAsync(createSmithersRunReader({ projectRoot }));
    try {
      const runningFooRuns = await reader.listRuns({
        workflowId: "foo",
        status: "running",
        limit: 10,
      });
  ...
  it("listRuns() returns Smithers run rows and filters by workflow id path/name", async () => {
    ...
    const reader = await maybeAsync(createSmithersRunReader({ projectRoot }));
    try {
      await expect(reader.listRuns()).resolves.toEqual(
      ...
      expect(fooRuns.map((run: { runId: string }) => run.runId)).toEqual([
        "run-main",
        "run-name-only",
      ]);
      expect(fooRuns).toEqual([
  ...
  it("getRunDetail() maps run, nodes, attempts, events, frames, cursor, and raw output rows", async () => {
    ...
    const reader = await maybeAsync(createSmithersRunReader({ projectRoot }));
    try {
      const detail = await (reader.getRunDetail as any)("run-main", {
        includeOutputs: true,
        eventLimit: 2,
        frameLimit: 5,
      });
      ...
      expect(detail.attempts).toContainEqual(
        expect.objectContaining({
          nodeId: "draft",
          attempt: 2,
          cached: true,
      ...
      expect(detail.frames).toEqual([
        expect.objectContaining({
          ...
          xmlHash: "xml-hash-1",
          encoding: "full",
          xmlJson: runMainFrameXmlJson,
          xml: runMainFrameXml,
          mountedTaskIdsJson: '["draft","review"]',
          mountedTaskIds: ["draft", "review"],
          taskIndexJson: runMainTaskIndexJson,
  ...
  it("exposes inflated latest delta frame xmlJson through Smithers adapter behavior", async () => {
    const projectRoot = tempProject("custom-harness-smithers-delta-frame-");
    createSmithersMappingFixtureDb(projectRoot);

    const { createSmithersRunReader } = await loadRunReader();
    const reader = await maybeAsync(createSmithersRunReader({ projectRoot }));
    try {
      const detail = await (reader.getRunDetail as any)("run-delta-frame", {
        frameLimit: 1,
      });

      expect(detail.frames).toEqual([
        expect.objectContaining({
          runId: "run-delta-frame",
          frameNo: 2,
          encoding: "delta",
          xmlHash: "xml-hash-delta-2",
          xmlJson: deltaFrameXmlJson,
          xml: deltaFrameXml,
          mountedTaskIds: ["review"],
          taskIndexJson: reviewFrameTaskIndexJson,
          taskIndex: [{ nodeId: "review", ordinal: 0, iteration: 0 }],
        }),
      ]);
      expect(detail.frames[0].xmlJson).not.toBe(reviewFrameDeltaJson);
      expect(detail.parseWarnings ?? []).toEqual([]);
    } finally {
      reader.close();
    }
  ...
  it("listEvents(afterSeq) returns newer DB events plus a monotonic cursor", async () => {
    ...
    const reader = await maybeAsync(createSmithersRunReader({ projectRoot }));
    try {
      const result = await (reader.listEvents as any)("run-main", {
        afterSeq: 1,
        limit: 10,
      });
  ...
  it("listEvents() returns parse warnings for malformed payload JSON without dropping the event", async () => {
    ...
    const reader = await maybeAsync(createSmithersRunReader({ projectRoot }));
    try {
      const result = await (reader.listEvents as any)("run-bad-json", {
        limit: 10,
      });
  ...
  it("listEvents() preserves the event cursor on empty pages after afterSeq", async () => {
    const projectRoot = tempProject(
      "custom-harness-smithers-events-empty-cursor-"
    );
    createSmithersMappingFixtureDb(projectRoot);
  ...
  it("malformed JSON fields produce field-level parse warnings and missing outputs are tolerated", async () => {
    const projectRoot = tempProject(
      "custom-harness-smithers-mapping-bad-json-"
    );
    createSmithersMappingFixtureDb(projectRoot);
    ...
    const reader = await maybeAsync(createSmithersRunReader({ projectRoot }));
    try {
      const detail = await (reader.getRunDetail as any)("run-bad-json", {
        includeOutputs: true,
      });
      ...
      expect(detail.parseWarnings).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({
            field: "run.configJson",
            runId: "run-bad-json",
          }),
          expect.objectContaining({
            field: "attempt.heartbeatDataJson",
            runId: "run-bad-json",
            nodeId: "bad-node",
          }),
          expect.objectContaining({
            field: "attempt.errorJson",
            runId: "run-bad-json",
            nodeId: "bad-node",
          }),
          expect.objectContaining({
            field: "attempt.metaJson",
            runId: "run-bad-json",
            nodeId: "bad-node",
          }),
          expect.objectContaining({
          ...
          expect.objectContaining({
            field: "frame.mountedTaskIdsJson",
            runId: "run-bad-json",
            frameNo: 1,
          }),
          expect.objectContaining({
            field: "frame.taskIndexJson",
            runId: "run-bad-json",
            frameNo: 1,
          }),
        ])
