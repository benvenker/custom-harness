      ...
      children: [
        {
          ...
          props: {},
          children: [
            { kind: 'element', tag: 'smithers:task', props: { id: 'collect-input' }, children: [] },
            { kind: 'element', tag: 'smithers:task', props: { id: 'write-summary' }, children: [] },
          ],
        },
      ],
    },
    tasks: [
      { nodeId: 'collect-input', label: 'Collect Input', ordinal: 0, prompt: 'Collect input' },
      { nodeId: 'write-summary', label: 'Write Summary', ordinal: 1, prompt: 'Write summary', dependsOn: ['collect-input'] },
    ],
  } as GraphSnapshot;
}
...
function fakeMetadataSnapshot(): GraphSnapshot {
  return {
      ...
      children: [
        {
          ...
          props: {},
          children: [
            { kind: 'element', tag: 'smithers:task', props: { id: 'editable-task' }, children: [] },
            { kind: 'element', tag: 'smithers:task', props: { id: 'plain-task' }, children: [] },
          ],
        },
      ],
    },
    tasks: [
      ...
      { nodeId: 'plain-task', label: 'Plain Task', ordinal: 1, prompt: 'Plain prompt' },
    ],
  } as GraphSnapshot;
}
...
describe('project workflow graph API', () => {
  it('returns not found for unknown workflow IDs before calling the renderer', async () => {
    ...
    const response = await handler(new Request('http://localhost/api/workflows/missing/graph'));
  ...
  it('renders a selected workflow through Smithers graph data without legacy runs artifacts', async () => {
    ...
    writeWorkflow(projectRoot, 'foo');
    const calls: Array<{ projectRoot: string; workflowId: string; workflowPath: string }> = [];
    const handler = createHarnessServerHandler({
      rootDir: process.cwd(),
      projectRoot,
      workflowId: 'foo',
    ...
    const body = await response.json() as { ok: boolean; workflowId: string; workflowPath: string; graph: { source?: { kind: string }; nodes: Array<{ id: string }>; edges: Array<{ from: string; to: string; label?: string }> } };
    expect(body.ok).toBe(true);
    expect(body.workflowId).toBe('foo');
    expect(body.workflowPath).toBe(join(resolve(projectRoot), '.smithers', 'workflows', 'foo.tsx'));
    ...
    expect(body.graph.edges).toContainEqual({ from: 'collect-input', to: 'write-summary', label: '' });
    expect(body.graph.edges).toContainEqual({ from: 'collect-input', to: 'write-summary', label: 'dependsOn' });
    expect(calls).toEqual([{ projectRoot: resolve(projectRoot), workflowId: 'foo', workflowPath: join(resolve(projectRoot), '.smithers', 'workflows', 'foo.tsx'), input: {}, outputs: {} }]);
    expect(existsSync(join(projectRoot, 'runs'))).toBe(false);
    expect(existsSync(join(projectRoot, '.poolside'))).toBe(false);
  });

  it('returns TaskDescriptor meta.studio metadata as structured graph JSON', async () => {
    ...
    const editableNode = body.graph.nodes.find((node) => node.id === 'editable-task');
    ...
    expect(body.graph.edges).toContainEqual({ from: 'editable-task', to: 'plain-task', label: '' });
  });
});
