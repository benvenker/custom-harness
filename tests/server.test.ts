...
import type {
  RunSmithersWorkflowOptions,
  RunSmithersWorkflowResult,
} from "../src/app/runSmithersWorkflow.js";
import type {
  RunOutcomeOptions,
  RunOutcomeResult,
} from "../src/app/runOutcome.js";
...
function writeExistingRun(runsDir: string, runId: string, plan: PlannerOutput) {
  writeExistingRunWithRawPlan(runsDir, runId, plan, {
    path: plan.path,
    reason: plan.reason,
  });
}
...
function writeExistingRunWithRawPlan(
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ raw: rawPlan, graph: { nodes: [] } }, null, 2)}\n`
  );
}
...
describe("HTTP server rerun API", () => {
  it("reruns exported Smithers graph plans through the Smithers execution path", async () => {
    ...
    let resolveSmithers: ((result: RunSmithersWorkflowResult) => void) | null =
      null;
    const handler = createHarnessServerHandler({
      rootDir: process.cwd(),
      runsDir,
      runOutcome: async (options): Promise<RunOutcomeResult> => {
        ...
        throw new Error(
          "planner rerun path should not be called for Smithers graph exports"
        );
      },
    ...
    const response = await handler(
      new Request("http://localhost/api/runs/source-run/rerun", {
        method: "POST",
        body: JSON.stringify({
    ...
    expect(smithersCalls[0]?.workflowPath).toBe(
      "/tmp/example/.smithers/workflows/custom-code-review.tsx"
    );
  ...
  it("forwards promptOverrides to the Smithers rerun and merges them with inherited overrides", async () => {
    ...
    const response = await handler(
      new Request("http://localhost/api/runs/parent-run/rerun", {
        method: "POST",
        body: JSON.stringify({
          runId: "forked-run",
          promptOverrides: {
  ...
  it("rejects promptOverrides with a 400 when the existing run is not a Smithers workflow", async () => {
    ...
    const handler = createHarnessServerHandler({
      ...
      runOutcome: async () => {
        throw new Error(
          "runOutcome should not be invoked when validation fails"
        );
      },
    });

    const response = await handler(
      new Request("http://localhost/api/runs/planner-run/rerun", {
        method: "POST",
  ...
  it("starts a rerun immediately by reusing its goal and raw planner output", async () => {
    ...
    const handler = createHarnessServerHandler({
      ...
      runOutcome: async (options): Promise<RunOutcomeResult> => {
        calls.push(options);
        const planned =
          typeof options.planner === "function"
...
describe("HTTP server Smithers create API", () => {
  it("starts a Smithers workflow run from workflowPath and input", async () => {
    ...
    const handler = createHarnessServerHandler({
      ...
      runOutcome: async (options): Promise<RunOutcomeResult> => {
        outcomeCalls.push(options);
        throw new Error(
          "planner run path should not be called for direct Smithers workflow runs"
        );
      },
      runSmithersWorkflow: async (
        options
  ...
  it("rejects invalid promptOverrides without starting a Smithers run", async () => {
    ...
    const handler = createHarnessServerHandler({
      ...
      runSmithersWorkflow: async (
        ...
        smithersCalls.push(options);
        throw new Error(
          "runSmithersWorkflow should not be invoked when validation fails"
        );
      },
    });
  ...
  it("requires workflowPath and input when creating a Smithers workflow run", async () => {
    ...
    const missingWorkflowPath = await handler(
      new Request("http://localhost/api/smithers-runs", {
        method: "POST",
    ...
    const missingInput = await handler(
      new Request("http://localhost/api/smithers-runs", {
        method: "POST",
        body: JSON.stringify({
          workflowPath:
    ...
    expect(await missingWorkflowPath.json()).toEqual({
      ok: false,
      error: "Missing workflowPath",
    });
    expect(missingInput.status).toBe(400);
    expect(await missingInput.json()).toEqual({
      ok: false,
      error: "Missing input",
    });
    expect(smithersCalls).toHaveLength(0);
  ...
  it("keeps POST /api/runs on the planner-backed run path", async () => {
    ...
    const handler = createHarnessServerHandler({
      ...
      runSmithersWorkflow: async (
        ...
        smithersCalls.push(options);
        throw new Error(
          "runSmithersWorkflow should not be invoked for POST /api/runs"
        );
      },
    });
