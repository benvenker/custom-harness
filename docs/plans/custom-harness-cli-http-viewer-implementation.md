...
Run-inspection endpoints should be added when they can read Smithers canonical SQLite/runtime state through `SmithersAdapter` or a dedicated read-only `SmithersRunReader`:

```txt
GET  /api/smithers/runs
GET  /api/smithers/runs/:id
GET  /api/smithers/runs/:id/events
```

In project-aware mode, run endpoints must not be backed by legacy `runs/index.json` as authoritative state. `runs/index.json` and `runs/<runId>/*.json` are compatibility artifacts only. Initial response shapes can be simple, but run existence/status should come from Smithers SQLite (`_smithers_runs`, with node/attempt/event/frame data from the related `_smithers_*` tables). API errors should use the same `{ ok:false, error:{ code, message, path?, flag?, fix? } }` shape as the CLI where practical.
...
Next after the first loop works:

- Add a read-only `SmithersRunReader` over Smithers SQLite/runtime APIs.
- Poll run status from `_smithers_runs` through that reader.
- Show node status from `_smithers_nodes` / `_smithers_attempts`.
- Show timeline/events from `_smithers_events`; logs can enrich this when present but are not authoritative.
