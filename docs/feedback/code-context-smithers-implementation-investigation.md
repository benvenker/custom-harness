...
5. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/smthrs/src/create.js` (lines 197-284) - `createSmithers` cwd/db behavior and DB/table side effects when importing workflows.
6. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/smthrs/src/bin/smithers.js` (lines 1-35) - local-bin delegation behavior relevant to `bunx smthrs` vs project-local runtime.
7. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/@smthrs/cli/src/workflows.js` (lines 15-116) - actual workflow discovery, ID validation, metadata parsing, scaffold output.
8. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/@smthrs/cli/src/index.js` (lines 50-82, 1449-1523, 1722-1822, 4181-4218, 4518-4589, 4778-4847) - actual CLI load/cwd, `up`, `workflow *`, `graph`, `replay`, `fork` implementations.
9. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/@smthrs/engine/src/engine.js` (lines 1248-1274, 3972-3999, 4938-5050) - root/log path defaults, `renderFrame`, run DB insertion and input persistence.
10. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/@smthrs/engine/src/events.js` (lines 176-205) - NDJSON `stream.ndjson` write implementation.
11. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/@smthrs/db/src/snapshot.js` (lines 55-76, 84-127) - Smithers graph loads persisted input/outputs by run id.
12. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/@smthrs/driver/src/SmithersCtx.js` (lines 38-118) - `SmithersCtx` output accessors and `outputMaybe` semantics.
13. `/Users/ben/code/agents/smithers/code-review/.smithers/node_modules/@smthrs/graph/src/types.ts` (lines 3-10, 129-190) - `GraphSnapshot`/XML/task shape.
...
Smithers is render/execute/persist/re-render, not a separate static DAG model. `docs/smithersai-smithers.txt` says each render extracts a `GraphSnapshot`, execution persists outputs, and later renders read persisted state (lines 567-606). `GraphSnapshot` itself is just `{ runId, frameNo, xml, tasks }` (`@smthrs/graph/src/types.ts` lines 185-190), with task descriptors carrying dependencies, prompt, agent, output table, approval flags, etc. (lines 129-176).
...
```js
// @smthrs/cli/src/workflows.js lines 15-77
...
`workflow run/list/path/create/doctor` all use `process.cwd()` as project root (`@smthrs/cli/src/index.js` lines 1722-1822). `up` and `graph` take workflow **file paths**, not IDs; `workflow run <name>` resolves IDs from `.smithers/workflows/<name>.tsx` (`docs/smithersai-smithers.txt` lines 6740-6750, 6927-6934, 7071-7096). Practical command pattern for a Poolside project root is therefore:

```bash
cd /path/to/project
bunx smthrs workflow list --format json
bunx smthrs workflow path weekly-slack-digest --format json
bunx smthrs graph .smithers/workflows/weekly-slack-digest.tsx --input '{...}' --format json
bunx smthrs workflow run weekly-slack-digest --input '{...}' --detach --format json
```

`CONTEXT.md` is right that docs prefer `bunx smthrs`, not bare `smithers` (`docs/smithersai-smithers.txt` lines 895-905). But the installed package's bin is named `smithers` (`smthrs/package.json` lines 33-35), and `src/bin/smithers.js` delegates to `./.smithers/node_modules/.bin/smithers` only when cwd is the **project root** (lines 1-35). I observed `cd /Users/ben/code/agents/smithers/code-review && bunx smthrs workflow list --format json` worked and discovered workflows; `cd .../.smithers && bunx smthrs workflow list --format json` failed from bunx temp deps in this environment, while `./node_modules/.bin/smithers workflow list` from inside `.smithers` returned `[]` because it looks for `.smithers/workflows` under cwd. Implication: run Smithers CLI from project root, not `.smithers/`, and pass `.smithers/workflows/<id>.tsx` for file-path commands.

`graph` implementation is render-only in the sense of no task execution/run insertion, but not side-effect-free:

```js
// @smthrs/cli/src/index.js lines 4189-4218
...
`createSmithers` opens/creates `./smithers.db` relative to `process.cwd()` by default and creates/migrates schema tables when the workflow module is imported (`smthrs/src/create.js` lines 197-284). `graph` also calls `ensureSmithersTables` (line 4194). I observed no new `.smithers/executions` files and `ps` stayed empty after `graph`, but DB/schema/WAL touch is possible. So the plan phrase “no side effects” should be softened: “does not execute tasks and does not insert a Smithers run; may import workflow and ensure/open DB tables.”
...
DB run state is primary. `runWorkflow` inserts `_smithers_runs` and input rows (`engine.js` lines 4938-5050); `_smithers_nodes`, `_smithers_attempts`, `_smithers_frames`, and `_smithers_events` hold node state, attempt history, frames, and event history (`@smthrs/db/src/sql-message-storage.js` lines 13-76 and 205-211); `loadOutputs` reads per-schema tables by `runId` (`snapshot.js` lines 84-127). NDJSON logs are observability, not identity.
...
Smithers has native fork/time-travel CLI surfaces: `replay` and `fork` take workflow file path, `--run-id`, `--frame`, optional input overrides/reset nodes/label, and can resume (`@smthrs/cli/src/index.js` lines 4518-4589 and 4778-4847). This challenges any alpha reliance on custom `forkedFrom` JSON as long-term fork semantics.
